...
/**
 * Created by Rent-A-Center on 1/28/2016.
 */
public class DBHandler extends SQLiteOpenHelper {
    ...
    public void addShoppingListItem(String name, double price, Integer quantity, Integer listId){
        ContentValues values = new ContentValues();
    ...
    public Cursor getShoppingListItem(Integer listId){

        SQLiteDatabase db = getWritableDatabase();

        return db.rawQuery("SELECT * FROM " + TABLE_SHOPPING_LIST_ITEM + " WHERE " + COLUMN_ITEM_LIST_ID + " = " + listId, null);
    ...
    public String getShoppingListName(int id){
        ...
        if (numShoppingLists >= 1) {
            cursor.moveToFirst();
            if ((cursor.getString(cursor.getColumnIndex("list_name")) != null)) {
                dbString = cursor.getString(cursor.getColumnIndex("list_name"));
            }
        }
    ...
    public int isItemPurchased(Integer itemId){
        ...
        String query = "SELECT * FROM " + TABLE_SHOPPING_LIST_ITEM +
                " WHERE " + COLUMN_ITEM_HAS + " = \"false\" " + "AND " +
    ...
    public ShoppingListItem getShoppingListItems(Integer itemId) {
        ...
        String query = "SELECT * FROM " + TABLE_SHOPPING_LIST_ITEM + " WHERE "  +
                COLUMN_ITEM_ID + " = " + itemId;
